...
import { Search, Camera, Upload, Loader2, X, AlertCircle, ScanSearch, Type } from 'lucide-react'
...
const MODES: { key: Mode; label: string; icon: React.ElementType }[] = [
  { key: 'text',  label: '이름 검색', icon: Type },
  { key: 'shape', label: '모양 검색', icon: ScanSearch },
  { key: 'image', label: '사진 검색', icon: Camera },
]

const RECENT_KEY = 'recentMedSearches'
...
export default function SearchPage() {
  ...
  const handleTextSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.items?.length) saveRecentSearch(query.trim())
      else toast('검색 결과가 없습니다', { icon: '🔍' })
  ...
  const searchByKeyword = (keyword: string) => {
    ...
    setResults([])
    setLoading(true)
    fetch(`/api/medications/search?q=${encodeURIComponent(keyword)}`)
      .then(r => r.json())
      .then(data => {
        setResults(data.items ?? [])
        if (data.items?.length) saveRecentSearch(keyword)
        else toast('검색 결과가 없습니다', { icon: '🔍' })
      })
      .catch(() => toast.error('검색 중 오류가 발생했습니다'))
  ...
  const handleShapeSearch = async () => {
    if (!selectedShape && !selectedColor1 && !selectedColor2 && !markText.trim() && !selectedForm) {
      toast('모양, 색상, 제형 중 하나 이상 선택해주세요', { icon: '💊' }); return
    }
    setLoading(true); setResults([])
    try {
  ...
  const resetShape = () => { setSelectedShape(''); setSelectedColor1(''); setSelectedColor2(''); setMarkText(''); setSelectedForm('') }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 p-1 bg-white dark:bg-sage-800 border border-sage-100 dark:border-sage-700 rounded-2xl shadow-sm w-fit">
        {MODES.map(({ key, label, icon: Icon }) => (
          <button
            key={key}
            onClick={() => { setMode(key); setResults([]) }}
            className={clsx(
        ...
        <div className="space-y-5">
          <form onSubmit={handleTextSearch} className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="inline-flex items-center gap-2 bg-mint-500 hover:bg-mint-600 text-white font-semibold px-5 py-3 rounded-2xl transition-colors shadow-md shadow-mint-500/20 whitespace-nowrap disabled:opacity-50"
            >
          ...
          {recentSearches.length > 0 && results.length === 0 && !loading && (
            <div className="space-y-2.5">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-sage-700 dark:text-sage-300">최근 검색</p>
                <button
                  onClick={clearRecentSearches}
                  className="text-xs text-sage-400 hover:text-sage-600 dark:hover:text-sage-300 transition-colors"
                >
                  전체 삭제
                </button>
              </div>
              <div className="flex flex-wrap gap-2">
                {recentSearches.map(term => (
                  <div key={term} className="flex items-center gap-1 bg-white dark:bg-sage-800 border border-sage-100 dark:border-sage-700 rounded-full px-3 py-1.5 shadow-sm">
                    <button
                      onClick={() => searchByKeyword(term)}
                      className="text-sm text-sage-700 dark:text-sage-200 font-medium hover:text-mint-600 dark:hover:text-mint-400 transition-colors"
                    >
                      {term}
                    </button>
                    <button
                      onClick={() => removeRecentSearch(term)}
                      className="ml-0.5 text-sage-300 hover:text-sage-500 dark:hover:text-sage-300 transition-colors"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </div>
      ...
      {mode === 'shape' && (
        <div className="bg-white dark:bg-sage-800 rounded-3xl border border-sage-100 dark:border-sage-700 shadow-sm p-6 space-y-6">
          ...
          <div>
            <p className="text-xs font-bold text-sage-400 tracking-widest uppercase mb-1">색상 1</p>
            <p className="text-xs text-sage-400 mb-3">주 색상을 선택하세요</p>
            <ColorPicker selected={selectedColor1} onChange={setSelectedColor1} />
          </div>

          <div>
            <p className="text-xs font-bold text-sage-400 tracking-widest uppercase mb-1">색상 2 <span className="font-normal normal-case text-sage-300">(선택)</span></p>
            <p className="text-xs text-sage-400 mb-3">투톤이면 두 번째 색도 선택하세요</p>
            <ColorPicker selected={selectedColor2} onChange={setSelectedColor2} />
          </div>

          <div>
            <p className="text-xs font-bold text-sage-400 tracking-widest uppercase mb-3">제형 <span className="font-normal normal-case text-sage-300">(선택)</span></p>
            <div className="flex flex-wrap gap-2">
              {FORM_CODES.map(form => (
                <button
                  key={form}
          ...
          <div>
            <label className="text-xs font-bold text-sage-400 tracking-widest uppercase mb-1 block">각인 문자 <span className="font-normal normal-case text-sage-300">(선택)</span></label>
            <p className="text-xs text-sage-400 mb-2">알약에 새겨진 문자나 숫자를 입력하세요</p>
            <input type="text" value={markText} onChange={e => setMarkText(e.target.value)}
              placeholder="예: 500, TY, DW" className="input-base" />
          </div>
